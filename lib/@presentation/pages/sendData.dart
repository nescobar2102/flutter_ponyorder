...
class SendataSincronizacion {
      ...
      for (int i = 0; i < data.length; i++) {
        final body = {
          ...
          "id_tipo_identificacion":
              data[i]['id_tipo_identificacion'].toString(),
          ...
          "segundo_nombre":
              data[i]['segundo_nombre'] != '' ? data[i]['segundo_nombre'] : '',
          "e_mail": data[i]['e_mail'],
          "telefono_celular": data[i]['telefono_celular'].toString(),
          "id_forma_pago":
              data[i]['id_forma_pago'] != '' ? data[i]['id_forma_pago'] : '01',
        ...
        final response = await http
            .post(Uri.parse('${Constant.URL}/nuevo_cliente_app'), body: body);

        var jsonResponse =
            convert.jsonDecode(response.body) as Map<String, dynamic>;
        var success = jsonResponse['success'];
        if (response.statusCode == 201 && success) {
          await OperationDB.updateCliente(
              data[i]['id_tercero'], data[i]['nit']);
        }
      }
    }
  ...
  static Future<void> createPedidoAPI() async {
    ...
    if (data != false) {
      for (int i = 0; i < data.length; i++) {
        ...
        final dataDet = await OperationDB.getPedidoDetFlagNO(
            numero, data[i]['id_empresa'], data[i]['id_tipo_doc'], nit);
          ...
          'pedidos': [
            {
              ...
              "id_direccion_factura":
                  data[i]['id_direccion_factura'].toString(),
        ...
        final response =
            await http.post(Uri.parse('${Constant.URL}/synchronization_pedido'),
  ...
  static Future createReciboApi() async {
    ...
    if (data != false) {
      for (int i = 0; i < data.length; i++) {
        ...
        if (response.statusCode == 201 && success) {
          await OperationDB.updateCuentasFlag(data[i]['numero'].toString(),
              data[i]['nit'], data[i]['tipo_doc']);
        } else {
  ...
  static Future<void> sendCartera() async {
    ...
    if (data != false) {
      for (int i = 0; i < data.length; i++) {
        final dataDet = await OperationDB.getCarteraDetFlagNO(
            data[i]['numero'].toString(),
            data[i]['id_empresa'],
            data[i]['id_tipo_doc'],
            data[i]['nit']);
        ...
        if (dataDet != false) {
            ...
            'cartera_proveedores': [
              {
                ...
                "total": data[i]['total'],
                "vencimiento":
                    convertDateFormat2(data[i]['vencimiento'].toString()),
                "letras": data[i]['letras'],
                "id_moneda": data[i]['id_moneda'],
                "id_tercero": data[i]['id_tercero'].toString(),
                "id_sucursal_tercero":
                    data[i]['id_sucursal_tercero'].toString(),
                "id_recaudador": data[i]['id_recaudador'].toString(),
                "fecha_trm":
                    convertDateFormat2(data[i]['fecha_trm'].toString()),
                "trm": "1",
                  ...
                  for (var j = 0; j < dataDet.length; j++) ...[
                    {
                      ...
                      "id_sucursal_tercero":
                          dataDet[j]['id_sucursal_tercero'].toString(),
                      "id_empresa_cruce":
                          dataDet[j]['id_empresa_cruce'].toString(),
                      "id_sucursal_cruce":
                          dataDet[j]['id_sucursal_cruce'].toString(),
                      "id_tipo_doc_cruce":
                          dataDet[j]['id_tipo_doc_cruce'].toString(),
                      "numero_cruce": dataDet[j]['numero_cruce'].toString(),
                      "fecha":
                          convertDateFormat3(dataDet[j]['fecha'].toString()),
                      ...
                      "documento_forma_pago":
                          dataDet[j]['documento_forma_pago'].toString(),
                      "distribucion": dataDet[j]['distribucion'],
                      "trm": dataDet[j]['trm'],
                      "id_recaudador": "6220948",
                      "id_suc_recaudador":
                          dataDet[j]['id_suc_recaudador'].toString(),
                      "fecha_trm": convertDateFormat2(
                          dataDet[j]['fecha_trm'].toString()),
          ...
          if (response.statusCode == 201 && success) {
            ...
            final response = await http.post(
                Uri.parse("${Constant.URL}/consecutivo_recibo_app"),
                body: (body));
            var jsonResponse =
                convert.jsonDecode(response.body) as Map<String, dynamic>;
            var success = jsonResponse['success'];
            ...
            if (response.statusCode == 200 && success) {
              await OperationDB.updateReciboFlag(data[i]['numero'].toString(),
                  data[i]['nit'], data[i]['id_tipo_doc']);
            }
          }
        }
      }
  ...
  static Future getMedioContactoSincronizacion() async {
    final response =
        await http.get(Uri.parse("${Constant.URL}/medioContacto_all"));
    var jsonResponse =
        convert.jsonDecode(response.body) as Map<String, dynamic>;
    var success = jsonResponse['success'];
    if (response.statusCode == 200 && success) {
  ...
  static Future getTipoIdentificacion() async {
    final response =
        await http.get(Uri.parse("${Constant.URL}/tipoidentificacion_all"));
    var jsonResponse =
        convert.jsonDecode(response.body) as Map<String, dynamic>;
    var success = jsonResponse['success'];
    if (response.statusCode == 200 && success) {
  ...
  static Future getTipoEmpresa() async {
    final response =
        await http.get(Uri.parse("${Constant.URL}/tipoempresa_all"));
    var jsonResponse =
        convert.jsonDecode(response.body) as Map<String, dynamic>;
    var success = jsonResponse['success'];
    if (response.statusCode == 200 && success) {
